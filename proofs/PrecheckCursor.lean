...
/-- Validation and gas charging preserve the program counter and stack. -/
theorem precheck_cursor {before after : Ethereum.State} {jumps : Array UInt256}
    {op : Operation} {cost : Nat} (run : Z jumps op before = .ok (after, cost)) :
    after.machineState.pc = before.machineState.pc ∧
      after.machineState.stack = before.machineState.stack := by
  unfold Z at run
  ...
  by_cases hjump :
      op = Operation.JUMP ∧
        Z.notIn state₁.machineState.stack[0]? jumps = true
