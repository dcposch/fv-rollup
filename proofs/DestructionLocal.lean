...
/-- Opcode validation and gas charging preserve the self-destruct set. -/
theorem precheck_destruct_set {before after : Ethereum.State} {validJumps : Array UInt256}
...
/-- Local instructions cannot schedule account deletion. -/
theorem local_step_destruct_set {before after : Ethereum.State} {gasCost : Nat}
...
/-- Self-destruct schedules only the executing account, when required by its creation history. -/
theorem selfdestruct_step_set {before after : Ethereum.State} {gasCost : Nat}
    {arg : Option (UInt256 × Nat)} (run : step gasCost (.SELFDESTRUCT, arg) before = .ok after) :
