...
private theorem mapping_return_memory (slot owner word : UInt256) :
    mappingReturnMem slot owner word =
      (solcMappingHashMem slot owner ++ ffi.ByteArray.zeroes 32) ++ UInt256.toByteArray word := by
...
private theorem mapping_return_read64 (slot owner word : UInt256) :
  ...
  have padded : (solcMappingHashMem slot owner ++ ffi.ByteArray.zeroes 32).size = 128 := by
