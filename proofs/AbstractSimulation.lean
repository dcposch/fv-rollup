...
theorem abstract_step_sound {cursor : AbstractCursor} {next : List AbstractCursor}
    ...
    (advance : cursor.successors op arg = some next)
  ...
  have inputLocked : ((opcodeInput before cost).sload ⟨6⟩).2 ≠ ⟨0⟩ := locked
  cases op
  case Push kind =>
    cases kind
    case PUSH0 =>
      simp only [AbstractCursor.successors, Option.some.injEq] at advance
      subst next
      exact ⟨_, List.mem_singleton_self _, abstract_push0_step represented run⟩
    all_goals
      cases arg with
      | none => simp [AbstractCursor.successors] at advance
      | some argument =>
        rcases argument with ⟨value, width⟩
        simp only [AbstractCursor.successors, bind, Option.bind, pure, Option.some.injEq] at advance
        subst next
  ...
  all_goals simp only [AbstractCursor.successors] at advance
  ...
  all_goals first
    ...
    | (simp only [step] at run
       ...
       | exact abstract_execSload (before := opcodeInput before cost) inputRepresented inputLocked evolved run
