...
/-- Each call opcode composes the actual child result or the proved rollup entry. -/
theorem call_instruction_tree_refines
  ...
  have senderSafe : self ≠ (site.message parent).sender ∨ site.value = ⟨0⟩ := by
    ...
    · exact .inr zero
  have effect : BoundaryRefines self keys checked.accountMap during.accountMap := by
    ...
    · by_cases receiver : AccountAddress.ofUInt256 site.recipient = self
      · have target := call_opcode_target checkedForeign arguments receiver
        ...
        exact call_helper_rollup_refines
          (before := { checked with executionEnv.depth := before.executionEnv.depth })
          ordinary checkedForeign arguments receiver enabled
          checkedReady checkedSafe access helper
      · apply call_site_tree_refines site parent during checkedReady checkedSafe senderSafe source
          (run := helper)
