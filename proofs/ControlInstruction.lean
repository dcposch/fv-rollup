...
theorem control_instruction_sound {cursor : AbstractCursor} {next : List AbstractCursor}
    {before after : Ethereum.State} {jumps : Array UInt256}
    (represented : cursor.denotes before)
    (advance : cursor.controlSuccessors ((decode before.executionEnv.code cursor.pc).getD (.STOP, none)).1
  ...
  have successors := control_step_sound
    (before := { checked with executionEnv.depth := before.executionEnv.depth })
    checkedRepresented (continuing_instruction_kind decoded run) advance opcode
